...
def help():
    ...
    showinfo(title="作者", message="(这只是一个简单的帮助)由于tk部分指令在3.8下无法正常调用，所以撤销和重做暂时实现不了，尝试修复后无果，为了保证代码的完整性，我们将会保留他们，但请你知道，它们没有任何用处(这是计划的一部分)\n哦，对了，我们在测试版总是会附带一个编译器，不过它似乎不太稳定，正式版是不会考虑增加任何一个测试项的=)\n祝你好运=）")
...
filemenu.add_command(label="保存(方案二)", accelerator="请全屏", command=save2)
...
#SYS TEST
filemenu = tk.Menu(menu)
filemenu.add_command(label="时间", accelerator="test", command=get_time)
filemenu.add_command(label="计算器", accelerator="test", command=math)
filemenu.add_command(label="自带调试工具(不稳定)", accelerator="test", command=SYS)
menu.add_cascade(label="测试项目(不会投入正式版)", menu=filemenu)
...
#创建画布
image = tk.PhotoImage()
canvas = tk.Canvas(top, bg='white', width=1, height=1)
